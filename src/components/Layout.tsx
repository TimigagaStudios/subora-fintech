...
const NavItem = ({ icon: Icon, label, isActive, onClick }: NavItemProps) => (
  ...
  >
    <Icon size={20} className={cn(isActive ? "text-accent" : "text-text-secondary group-hover:text-text-primary")} />
    <span>{label}</span>
    ...
    <div className="flex h-screen bg-background text-text-primary overflow-hidden selection:bg-accent/30">
      ...
      <aside className="hidden md:flex flex-col w-72 border-r border-border bg-secondary-background/30 backdrop-blur-xl p-6 relative">
        <div className="flex items-center gap-3 mb-10 px-2">
          ...
          <span className="text-2xl font-black tracking-tight text-text-primary italic">Subora</span>
        </div>
        ...
        <div className="mt-auto space-y-4 pt-6">
          <div className="bg-accent/5 rounded-2xl p-4 border border-accent/10">
            <div className="flex items-center justify-between mb-2">
              <span className="text-xs font-semibold text-accent uppercase tracking-wider">Pro Plan</span>
              <TrendingUp size={14} className="text-accent" />
            </div>
            <p className="text-xs text-text-secondary mb-3 leading-relaxed font-medium">Unlock premium virtual cards and 5% cashback.</p>
          ...
          <div className="flex items-center gap-3 px-2 pt-4 border-t border-border group cursor-pointer">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm font-bold text-text-primary truncate">{user?.email || 'Alex Carter'}</p>
              <p className="text-[10px] font-bold text-accent truncate uppercase tracking-tighter">Premium User</p>
            </div>
      ...
      <main className="flex-1 overflow-y-auto overflow-x-hidden relative">
        ...
        <header className="sticky top-0 z-20 h-20 bg-background/80 backdrop-blur-md border-b border-border px-6 md:px-10 flex items-center justify-between">
          <div className="flex md:hidden items-center gap-3">
             ...
             <span className="text-xl font-black tracking-tight text-text-primary italic">Subora</span>
          </div>
        ...
        <div className="fixed inset-0 z-50 bg-background md:hidden">
          <div className="p-6 flex flex-col h-full">
            <div className="flex items-center justify-between mb-12">
              ...
              <button onClick={() => setIsMobileMenuOpen(false)} className="text-text-primary"><X size={24} /></button>
            </div>
            ...
            <div className="pb-6 border-t border-border pt-6 mt-4">
               <button className="flex items-center gap-3 w-full px-4 py-3 text-red-400 font-bold">
                  <LogOut size={20} />
                  <span>Log Out</span>
               </button>
            </div>
